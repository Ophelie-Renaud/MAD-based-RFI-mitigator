...
typedef struct {
    ...
    XMem_read_top_rfi_c_uio_map maps[ MAX_UIO_MAPS ];
} XMem_read_top_rfi_c_uio_info;

/***************** Variable Definitions **************************************/
static XMem_read_top_rfi_c_uio_info uio_info;
...
static int uio_info_read_name(XMem_read_top_rfi_c_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMem_read_top_rfi_c_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMem_read_top_rfi_c_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMem_read_top_rfi_c_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMem_read_top_rfi_c_Initialize(XMem_read_top_rfi_c *InstancePtr, const char* InstanceName) {
	XMem_read_top_rfi_c_uio_info *InfoPtr = &uio_info;
